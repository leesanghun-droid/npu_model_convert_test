...
static void _load_input_meta()
{
    ...
    input_meta_tab[0].image.scale = 0.00390625;
...
*/
vsi_status vnn_PreProcessGenderh5_ImageProcess
    (
    vsi_nn_graph_t *graph,
    const char **inputs,
    uint32_t input_num
    )
{
    ...
    {
        printf("vnn_PreProcessGenderh5_ImageProcess Only support JPG image file\n");
        status = VSI_FAILURE;
        TEST_CHECK_STATUS(status, final);
    }
...
vsi_status vnn_PreProcessGenderh5
    (
    vsi_nn_graph_t *graph,
    const char **inputs,
    uint32_t input_num
